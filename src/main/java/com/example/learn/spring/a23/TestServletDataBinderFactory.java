...
//        ServletRequestDataBinderFactory factory = new ServletRequestDataBinderFactory(List.of(method), initializer);
        ...
        ApplicationConversionService service = new ApplicationConversionService();  // 配合@DateTimeFormat处理了时间参数解析绑定
        ConfigurableWebBindingInitializer initializer = new ConfigurableWebBindingInitializer();
        initializer.setConversionService(service);

        // *** 此处对应A21.java的要点2：准备对象绑定与类型转换工厂
        ServletRequestDataBinderFactory factory = new ServletRequestDataBinderFactory(null, initializer);
    ...
    static class MyController {
        ...
        public void aaa(WebDataBinder dataBinder) {
            ...
            // 上面方法内部会使用FormatterPropertyEditorAdapter适配器将formatter转换为PropertyEditor
        }
    }
