package com.example.learn.spring.a33;
...
public class WebConfig_1 {
    ...
    static class MyHandlerMapping implements HandlerMapping {
        ...
        public HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
            ...
            return new HandlerExecutionChain(controller);  // 执行链
        }

        @Autowired
        private ApplicationContext context;
        private Map<String, Controller> collect;  // 路径和Controller映射关系

        @PostConstruct
        public void init() {
            collect = context.getBeansOfType(Controller.class).entrySet()
                    .stream().filter(e -> e.getKey().startsWith("/"))  // 找到容器中所有beanName为"/"开头的Controller
                    .collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
            System.out.println(collect);
        }
    }

    // 仿造SimpleControllerHandlerAdapter功能
    @Component
